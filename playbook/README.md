...
Ansible playbook для автоматической установки и настройки ClickHouse (система управления базами данных), Vector (сборщик и маршрутизатор логов) и Lighthouse (веб-интерфейс для мониторинга) на удаленных хостах.
...
- **Lighthouse** — последняя версия из репозитория VKCOM/lighthouse
...
- Настройка Nginx для раздачи статических файлов Lighthouse
...
- `group_vars/vector/lighhouse.yml` - (url репозитория с кодом, пути для размещения lighthouse на сервере и др.)
...
| `lighthouse` | lighthouse | Установка и настройка Lighthouse (установка Git и Nginx, клонирование репозитория, настройка Nginx, запуск сервиса) |
...
<img src="img/ans_3.png" width = 100%>